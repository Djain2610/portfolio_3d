...
// Add timeout handling for slow networks (30 seconds)
...
// Wrapper function to add timeout handling
function withLoadTimeout(promise, url, timeoutMs = LOAD_TIMEOUT) {
  return Promise.race([
    promise,
    new Promise((_, reject) =>
      setTimeout(
        () => reject(new Error(`[Load Timeout] Failed to load ${url} within ${timeoutMs}ms`)),
        timeoutMs
      )
    ),
  ]);
...
export function useSafeGLTF(url) {
  try {
    ...
    // Wrap the promise with timeout handling
    if (result && typeof result.then === 'function') {
      result
        .then((data) => {
          console.log(`[useSafeGLTF] Successfully loaded ${url}`);
          return data;
        })
        .catch((err) => {
          const errorDetails = {
          ...
          console.error(`[useSafeGLTF] Failed to load ${url}:`, errorDetails);
          
          // Log network info for debugging
          if (typeof window !== 'undefined' && navigator.connection) {
            console.warn(`[useSafeGLTF] Network info:`, {
